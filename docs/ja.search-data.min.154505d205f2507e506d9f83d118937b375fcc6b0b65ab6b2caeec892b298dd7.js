...
実行 # Telloと接続してから、展開したフォルダに含まれている Scratch3-Tello.exe を起動してください。
`}),e.add({id:1,href:"/ja/docs/development/build-env/",title:"環境構築",section:"開発",content:" 環境構築 # scratch3-tello リポジトリには scratch-vm と scratch-gui リポジトリに対して変更を行った差分がファイル単位で格納されています。そのため、このリポジトリをクローンしただけではビルドすることができません。\nRequirements # wget git node.js npm 作業用フォルダの作成 # $ mkdir scratch3-tello $ cd scratch3-tello 環境構築用スクリプトをダウンロードする # $ wget https://raw.githubusercontent.com/kebhr/scratch3-tello/master/build.sh $ chmod +x build.sh 環境構築用スクリプトを実行する # $ ./build.sh build.sh を実行することで、自動的に scratch-vm, scratch-gui, scratch-desktop がクローンされ、scratch3-tello で管理されている差分のファイルが適用されます。\nScratch3-Tello の起動 # $ cd scratch-desktop $ npm start パッケージを作成 # $ cd scratch-desktop $ npm run dist "}),e.add({id:2,href:"/ja/docs/getting-started/macos/",title:"macOSの場合",section:"導入方法",content:` macOSの場合 # ダウンロード # GitHubのReleasesから、macos.zip をダウンロードします。
展開 # ダウンロードしたzipファイルを任意の場所に展開してください。
実行 # Telloと接続してから、展開したフォルダに含まれている Scratch3-Tello.app を起動してください。
初回のみ、macOSのセキュリティ機能により、ダブルクリックでアプリを開くことはできません。 Controlキーを押しながらファイルをクリックし、表示されたメニューから「開く」を選択することで、開くことができます。
`}),e.add({id:3,href:"/ja/docs/development/i18n/",title:"多言語対応",section:"開発",content:` 多言語対応 # ブロック名は scratch-vm/src/extensions/scratch3_tello/index.js に定義されています。このファイルに記述されている message オブジェクトが i18n に使用されます。
message['内部ブロックコード']['言語コード'] が、そのブロックのその言語でのブロック名です。
翻訳の追加・更新に関して、随時Pull-Requestを受け付けています。
例 # 離陸する ブロックをドイツ語に翻訳します。現時点では、message['takeoff'] は次のようになっています。
const message = { takeoff: { \u0026#39;ja\u0026#39;: \u0026#39;離陸する\u0026#39;, \u0026#39;ja-Hira\u0026#39;: \u0026#39;りりくする\u0026#39;, \u0026#39;en\u0026#39;: \u0026#39;takeoff\u0026#39;, }, ... }; ドイツ語を表す言語コードは de です。message['takeoff']['de'] が abfliegen となるように、次のように編集します。
const message = { takeoff: { \u0026#39;ja\u0026#39;: \u0026#39;離陸する\u0026#39;, \u0026#39;ja-Hira\u0026#39;: \u0026#39;りりくする\u0026#39;, \u0026#39;en\u0026#39;: \u0026#39;takeoff\u0026#39;, \u0026#39;de\u0026#39;: \u0026#39;abfliegen\u0026#39;, }, ... }; `}),e.add({id:4,href:"/ja/docs/getting-started/linux/",title:"Linuxの場合",section:"導入方法",content:` Linuxの場合 # 現在のところ、Linux向けのビルド済みバイナリは提供していません。Linux上で動作させる場合は、環境構築#パッケージを作成 を読み、ご自身でビルド・パッケージングしてください。
...
`}),e.add({id:6,href:"/ja/docs/contact/",title:"お問い合わせ",section:"Docs",content:` お問い合わせ # お問い合わせはこちらのフォームよりお受けしております。
